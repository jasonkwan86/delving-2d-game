...
func generate_boss_arena() -> void:
	herringbone_wang_data_set.copy_hbwt_horizontal_to_tile_map_layer(Vector2i(0, -2), Vector2i(3, 12), world_tilemap)

func generate_world_borders(offset: Vector2i, dimensions: Vector2i) -> void:
	for row in dimensions.y:
		@warning_ignore("integer_division")
		herringbone_wang_data_set.copy_hbwt_horizontal_to_tile_map_layer(Vector2i(-2, -1), offset + Vector2i.DOWN * row + Vector2i.LEFT, world_tilemap)
		herringbone_wang_data_set.copy_hbwt_horizontal_to_tile_map_layer(Vector2i(-2, -1), offset + Vector2i.DOWN * row + Vector2i.RIGHT * (dimensions.x - 1), world_tilemap)
	for col in dimensions.x:
		herringbone_wang_data_set.copy_hbwt_horizontal_to_tile_map_layer(Vector2i(-2, -1), offset + Vector2i.DOWN * (dimensions.y - 1) + Vector2i.RIGHT * col, world_tilemap)
	#herringbone_wang_data_set.copy_hbwt_horizontal_to_tile_map_layer(Vector2i(-2, -1), Vector2i(3, -5), world_tilemap)
